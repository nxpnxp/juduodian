...
class AdminController extends Controller {
	 ...
	 * 门店审核
	 */
	private function diansh($Model,$ids){
		if($Model == 'Document'){
			$ids = explode(',', $ids);
			foreach ($ids as $k => $v) {
				$d = M($Model)->find($v);
				if(!$d){ continue; }				
				$uid = $d['uid'];
				$user = M('WxuserCode')->find($uid);
				if(!$user){ continue; }	
				if($user['shopid'] == '0'){
					$res = M('WxuserCode')->where('id='.$uid)->save(array('shopid'=>$v));
					//file_put_contents('test1.txt', json_encode($res).'--'.$v);
				}else{
					$shopids = explode(',', $user['shopid']);
					$shopids = array_merge($shopids,array($v));
					$shopids = array_unique($shopids);
					$shopids = implode(',', $shopids);
					$res = M('WxuserCode')->where('id='.$uid)->save(array('shopid'=>$shopids));
					//file_put_contents('test2.txt', json_encode($res).'--'.$shopids);
				}
			}
		}
	}
